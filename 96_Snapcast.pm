################################################################
#
#  $Id: 96_Snapcast.pm 22532 2022-06-27 Beta-User $
#
...
sub Snapcast_Initialize {
    ...
    $hash->{AttrList} =
          "streamnext:all,playing constraintDummy constraints disable:1 volumeStepSize volumeStepSizeSmall volumeStepSizeThreshold " . $readingFnAttributes;
    return;
}
...
sub Snapcast_Define {
    ...
    $hash->{ip}   = $arr[2] // 'localhost'; 
    $hash->{port} = $arr[3] // '1705'; 
    $hash->{MODE} = 'server';
    readingsSingleUpdate($hash,'state','defined',1);
    RemoveInternalTimer($hash);
    DevIo_CloseDev($hash);
...
sub Snapcast_Ready {
    ...
    for my $reading (grep { m{\A(?:streams|clients)}m }
                            #keys %{$hash{READINGS}} ) {
                            keys %{$hash->{READINGS}} ) {
...
sub Snapcast_updateClient {
  ...
  readingsBeginUpdate($hash) if !$noendupd;
  ...
  readingsEndUpdate($hash,1) if !$noendupd;
...
sub Snapcast_setClient {
  ...
  return if !defined $Snapcast_clientmethods{$param};
